...
# ╔═╡ 43f631ba-0eb1-4b7c-a7a3-2511da19f6e9
size([1,2,3])
...
# ╔═╡ df23e8db-c4ad-44dd-bed4-ed932834108d
function get_reds2(image::AbstractMatrix)
	# your code here!
	image2 = zeros(N0f8, size(image)[1], size(image)[2])
	for i in 1:size(image)[1]
		for j in 1:size(image)[2]
			image2[i,j] = get_red(image[i,j])
		end
	end
	return image2
end
...
# ╔═╡ 6be5e1cb-a738-4823-8982-259677d1d92f
get_reds2(philip_head)

# ╔═╡ 0e9d77c2-863f-41ca-b437-a0f20c7d1e6b
get_red(philip_head[1,2])
...
function matrix_to_image(matrix)
	matrix = value_as_color.(matrix)
	return matrix
end

# ╔═╡ e1b0bada-54b2-4050-91f0-b1cffc742b9c
matrix_to_image(get_reds(philip_head))
...
# ╔═╡ 63228ffa-4435-4105-86c1-07b964953538
function get_blue(pixel::AbstractRGB)
	# your code here!
	return pixel.b
end

# ╔═╡ 01056861-b523-4ce8-8e6f-125fac331661
...
# ╔═╡ ade2b646-1ed3-4cb0-b64a-79db6f8a8037
